    ...
    <div className="max-w-5xl mx-auto space-y-4 md:space-y-6">
      <div className="flex items-center justify-between gap-2">
        ...
        <motion.button
          ...
          className="flex items-center gap-2 px-3 md:px-4 py-2 bg-orange-500 text-white rounded-xl font-mono text-sm min-h-[44px] active:scale-95"
        >
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-4 md:p-5">
          ...
          <p className="font-mono text-2xl md:text-3xl font-bold text-orange-400">€{monthTotal.toFixed(2)}</p>
          <p className="text-xs text-muted-foreground mt-1">{monthPayments.length} payments</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-card border border-border rounded-2xl p-4 md:p-5 flex items-center justify-between">
          <div>
