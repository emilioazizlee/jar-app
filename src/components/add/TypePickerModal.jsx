...
import { CheckSquare, DollarSign, RefreshCw, CreditCard, Users, FileText, Target, User, Sparkles, FolderOpen } from 'lucide-react';
  ...
  if (selectedType === 'project') {
    return (
      <NewProjectModal
        open={open}
        onClose={handleClose}
        onCreated={(project) => { handleClose(); navigate(`/project/${project.id}`); }}
      />
    );
  }
      ...
      <DialogContent className="bg-card border-border max-w-lg">
        ...
        <div className="grid grid-cols-3 gap-3 pt-2">
          ...
          <motion.button
            ...
            className="flex flex-col items-center gap-2 p-4 rounded-xl bg-muted/50 border border-border hover:border-primary/40 transition-all group"
          >
            <div className="w-10 h-10 rounded-lg flex items-center justify-center transition-all group-hover:scale-110" style={{ backgroundColor: '#39ff1415' }}>
              <FolderOpen className="w-5 h-5 text-primary" />
            </div>
            <span className="font-mono text-xs text-muted-foreground group-hover:text-foreground transition-colors">Project</span>
          </motion.button>
          {/* Custom note */}
          <motion.button
            initial={{ opacity: 0, y: 10 }}
          ...
          >
            ...
            <span className="font-mono text-xs text-muted-foreground group-hover:text-foreground transition-colors">Custom</span>
          </motion.button>
        </div>
      </DialogContent>
    </Dialog>
